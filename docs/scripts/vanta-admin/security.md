...
## A note on protecting your code

Client and NUI files are downloaded to every player's cache and can always be
extracted — so **never** put secrets or security logic in them. To protect your
intellectual property when selling, use the **Cfx.re Asset Escrow** system
(the resource already includes an `escrow_ignore` block keeping `config.lua` and
`locales.lua` open for buyers).
